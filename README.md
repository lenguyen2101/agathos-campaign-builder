...
A working front-end prototype of the **Campaign** layer and the **Homepage Hero**
carousel control, built to match the existing Agathos admin portal (Ant Design
Pro shell) already used by the Project / Event / Organization layers.
...
**A homepage slide is live only while its campaign is running.** The carousel is
assembled by joining the stored `order` against the campaigns that currently
have `promoHero` on. A campaign slide reaches the public homepage only when:
...
visible ones, so a campaign that goes live again returns to the carousel instead
of vanishing. Campaigns newly switched on are appended. See `heroSlides()` and
`slideLive()`.
...
- `localStorage` persistence, and the `STORE_KEY` / `HERO_KEY` version bumps
  (`agathos.campaigns.v8`, `agathos.hero.v1`). The version suffix exists so stale
  local data reseeds during prototyping.
- Every **Reset** button, `resetStore()` and `resetHero()`.
